...
## Phase 4 — 심판 스트리밍 판정
- [x] 양쪽 도착 후 심판 `streamText` (채팅 스트리밍)
- [x] 판정에 **승자** + **결정적 논거** 포함
- [x] `onChatMessage` 또는 `saveMessages`로 UI 채팅에 표시
- **체크포인트:** 판정 메시지에 승자 이름과 구체적 논거가 포함됨
- **수정:** `Output.object` 파싱 실패 대비 `generateStructured` fallback(JSON 텍스트 + Zod + repair)
- **상태:** OK ✓

## Phase 5 — UI + E2E
- [x] 양쪽 대변인 진행 상황 실시간 표시
- [x] 주장(opening / 3 arguments / closing) 요약 표시
- [x] 채팅으로 심판 판정 확인
- [x] 부먹 vs 찍먹 제안 칩으로 입장 자동 추출 확인 가능
- [x] `debugAdvocate` 등 임시 callable 정리
- **체크포인트:**
