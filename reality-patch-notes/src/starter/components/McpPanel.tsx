...
export function McpPanel({
  ...
  onAddServer: (name: string, url: string) => Promise<unknown>;
  onRemoveServer: (serverId: string) => Promise<unknown>;
}) {
...
export function useMcpState(enabled: boolean) {
  ...
  const onMcpUpdate = useCallback(
    (state: MCPServersState) => {
      if (enabled) setMcpState(state);
    },
    [enabled]
  );

  return { mcpState, onMcpUpdate };
}
